...
module.exports = function CloudFormationDeployer(cloudFormationClient, bucketManager, deploymentBucket) {
	...
	this.deployStackSetTemplate = async function(accountId, template, options = {}, parameters = {}, bucketDeploymentKey) {
		...
		const templateUrl = `${bucketDeploymentKey}/${options.stackSetName}.cloudformation-innerstack.template`;
		await new Promise((resolve, reject) => {
			Tmp.file(async (err, path) => {
				if (err) {
					reject(err);
		...
		if (!stackExists) {
			console.log('Create stack set because it does not exist.');
			...
			stackParameters.Accounts = [accountId];
			stackParameters.Regions = options.regions;
			...
			for (let iteration = 0; iteration < 480; iteration++) {
				...
				if (operation.StackSetOperation.Status === 'FAILED') {
					throw { title: 'Failed to Update stack set', operation };
				}
				await new Promise(resolve => setTimeout(resolve, 15000));

				if (iteration > 400) {
					throw { title: 'Failed to update stack set.' };
				}
			}
		}
		...
		if (createParams.Regions.length) {
			...
			for (let iteration = 0; iteration < 480; iteration++) {
				...
				await new Promise(resolve => setTimeout(resolve, 15000));
				if (iteration > 400) {
					throw { title: 'Failed to create instances for stack that do not exist yet' };
				}
			}
		}
