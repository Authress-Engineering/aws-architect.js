module.exports = function CloudFormationDeployer(cloudFormationClient) {
	...
	this.waitForChangeSetCreation = function(stackName, changeSetName) {
		...
		timeout.setTime(timeout.getTime() + timeoutLength);
		let getTimeoutPromise = () => new Promise((resolve, reject) => setTimeout(() => resolve(), 10 * 1000));
		let maxErrors = 5;
		let totalErrors = 0;
		...
		// start waiting after 20 seconds
		return new Promise((resolve, reject) => setTimeout(() => resolve(), 5 * 1000)).then(() => iteratePromise());
	};
	...
	this.deployTemplate = function(template, options = {}, parameters = {}) {
		...
		.then(stackExists => {
			...
			.then(() => {
				return this.waitForChangeSetCreation(options.stackName, options.changeSetName);
			})
			.then(() => {
