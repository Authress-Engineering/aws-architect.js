...
class CloudFormationDeployer {
  ...
  async deployTemplate(template, options = {}, parameters = {}, bucketDeploymentKey) {
    ...
    if (stackExists) {
      ...
      if (isEqual(JSON.parse(currentStackTemplate.TemplateBody.trim()), JSON.parse(templateString.trim())) && currentStackData.Parameters.every(p => parameters[p.ParameterKey] === p.ParameterValue)) {
  ...
  async deployStackSetTemplate(accountId, template, options = {}, parameters = {}, bucketDeploymentKey) {
    ...
    if (stackExists && !createParams.Regions.length && existingStacks.every(s => s.Status === 'CURRENT')) {
      ...
      if (isEqual(JSON.parse(currentStackTemplate.TemplateBody.trim()), JSON.parse(templateString.trim()))
        && currentStackData.Parameters.every(p => parameters[p.ParameterKey] === p.ParameterValue)) {
