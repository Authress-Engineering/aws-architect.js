...
module.exports = function CloudFormationDeployer(cloudFormationClient, bucketManager, deploymentBucket) {
  ...
  this.validateTemplate = async function(template, stackName, bucketDeploymentKey) {
  ...
  this.waitForCompletion = async function(stackName, allow_update_rollback) {
    let timeout = new Date();
    let timeoutLength = 60 * 60 * 1000;
    timeout.setTime(timeout.getTime() + timeoutLength);
    let nextWaitTime = 10;
    let getTimeoutPromise = () => new Promise(resolve => setTimeout(() => resolve(), Math.min((nextWaitTime *= 2), 300) * 1000));
    let maxErrors = 5;
    let totalErrors = 0;
    ...
    // start waiting after 10 seconds stack takes a litle while to pick up changes
    await new Promise(resolve => setTimeout(() => resolve(), 10 * 1000));
    await iteratePromise();
  };

  this.waitForChangeSetCreation = async function(stackName, changeSetName) {
    let timeout = new Date();
  ...
  this.deployTemplate = async function(template, options = {}, parameters = {}, bucketDeploymentKey) {
    ...
    if (stackExists) {
      const currentStackTemplate = await this.cloudFormationClient.getTemplate({ StackName: options.stackName }).promise();
      if (JSON.stringify(currentStackTemplate.TemplateBody) === templateString) {
  ...
  this.stackSetExists = async function(stackSetName) {
    try {
  ...
  this.deployStackSetTemplate = async function(accountId, template, options = {}, parameters = {}, bucketDeploymentKey) {
    if (template === null) { throw { error: '{template} object must be defined.' }; }
    ...
    const existingRegions = await this.cloudFormationClient.listStackInstances({ StackSetName: options.stackSetName }).promise().then(data => data.Summaries.map(s => s.Region));
    const createParams = {
      StackSetName: options.stackSetName,
      Accounts: [accountId],
    ...
    if (createParams.Regions.length) {
      console.log(`Create stack set instances in regions: ${createParams.Regions.join(', ')}`);
