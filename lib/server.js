			...
			api.all(expressResource, async (req, res) => {
				...
				if (req.method !== 'OPTIONS') {
					try {
						...
						Object.keys(contextClone).forEach(p => {
				...
				if (response.headers) {
					Object.keys(response.headers).filter(h => h.match(/location/i)).forEach(header => {
						response.headers[header] = response.headers[header].replace(/^https?:\/\/(localhost:\d{2,5})\/(.*)$/g, 'http://$1/api/$2');
					});
