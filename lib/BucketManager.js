...
BucketManager.prototype.EnsureBucket = function(serviceTag, region){
	...
	.catch(data => {
		if(data.code !== 'NotFound') {
			return Promise.reject({ Title: 'Bucket already exists, but you do not have access to it.', Error: data.stack || data.toString(), Details: data });
		}
		var params = {
			Bucket: this.Bucket
		};
		...
		.catch(data => {
			switch (data.code) {
				case 'AccessDenied':
					return Promise.reject({ Title: 'Access Denied to bucket, check user role permissions.', BucketName: this.Bucket });
				case 'Forbidden':
					return Promise.reject({ Title: 'Bucket already exists in another account.', BucketName: this.Bucket });
				case 'InvalidBucketName':
					return Promise.reject({ Title: 'Invalid Bucket Name', BucketName: this.Bucket, Details: 'http://docs.aws.amazon.com/AmazonS3/latest/dev/BucketRestrictions.html'});
				default:
					return Promise.reject({ Title: 'AWS error creating bucket', Error: data.stack || data.toString(), Details: data });
			}
		});
	})
	.then(() => {
...
BucketManager.prototype.Deploy = function(contentPath, version) {
...
BucketManager.prototype.CopyBucket = function(source, target) {
	...
	.then(data => {
		if(data.IsTruncated) { return Promise.reject({Title: 'Failed to copy source', Error: 'Too many objects present.' }); }
