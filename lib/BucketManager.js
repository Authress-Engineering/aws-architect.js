...
class BucketManager {
	...
	async Deploy(contentPath, version, cacheControlRegexMap = [], contentTypeMappingOverride = {}) {
		...
		for (const file of sortedList.concat(indexList)) {
			...
			let matchingCacheMap = Array.isArray(cacheControlRegexMap) && cacheControlRegexMap.find(m =>
				!m.explicit && !m.regex || m.explicit === relativePathUnixFormat || m.regex && m.regex.exec && m.regex.exec(file)
			);
			...
			const putObjectParams = {
				...
				// ContentMD5: createHash('md5').update(Buffer.from(fileData)).digest('hex'),
				ContentType: contentTypeMapping[relativePath] || contentTypeMapping[path.extname(file)] || contentTypeMapping.default || 'text/plain',
			...
			try {
				...
				if (unixifiedIndexFilePath.match(/\/index.html$/)) {
					...
					putObjectParams.Body = fs.createReadStream(file);
					putObjectParams.Key = encodeURIComponent(`${this.unixify(path.join(version, redirectFileUrl))}/`);
					await this.S3Manager.putObject(putObjectParams).promise();
					console.log(`   => ${decodeURIComponent(putObjectParams.Key)} (Directory Handler)`);
					...
					putObjectParams.Key = encodeURIComponent(this.unixify(path.join(version, redirectFileUrl)));
					// putObjectParams.ContentMD5 = createHash('md5').update(Buffer.from(updatedRedirectFile)).digest('hex');
					await this.S3Manager.putObject(putObjectParams).promise();
					console.log(`   => ${decodeURIComponent(putObjectParams.Key)} (Redirect)`);
				}
			} catch (failure) {
