...
const {
  HeadBucketCommand, CreateBucketCommand, PutPublicAccessBlockCommand, PutBucketLifecycleConfigurationCommand,
  ListObjectsV2Command, DeleteObjectCommand, HeadObjectCommand, PutObjectCommand, CopyObjectCommand
...
class BucketManager {
  constructor(s3Manager, bucket, region) {
  ...
  async deletePath(directory) {
    ...
    const result = await this.S3Manager.send(new ListObjectsV2Command(params));
    await Promise.all(result.Contents.map(async object => {
      await this.S3Manager.send(new DeleteObjectCommand({ Bucket: this.Bucket, Key: object.Key }));
    }));
  }
  ...
  async putObjectIfDifferent(params) {
    try {
      const currentObject = await this.S3Manager.send(new HeadObjectCommand({ Bucket: params.Bucket, Key: params.Key }));
    ...
    await this.S3Manager.send(new PutObjectCommand(params));
  }
  ...
  CopyBucket(source, target) {
    ...
    return this.S3Manager.send(new ListObjectsV2Command(listParams))
    .then(data => {
      ...
      return data.Contents.map(item => item.Key).filter(key => !key.match(/\/$/)).reduce((listPromise, key) => {
        return listPromise.then(() => {
          return this.S3Manager.send(new CopyObjectCommand({
            Bucket: this.Bucket,
