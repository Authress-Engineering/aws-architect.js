...
LambdaManager.prototype.PublishNewVersion = async function(functionName, bucket, deploymentKey) {
  const result = await this.LambdaFactory.updateFunctionCode({
  ...
  // Wait for the lambda to become action before allowing it to be used anywhere.
...
LambdaManager.prototype.cleanupProduction = async function(functionName, forceRemovalOfAliases = false, dryRun = true) {
  ...
  console.log(`Total code size of all versions in ${this.LambdaFactory.config.region}: ${Math.floor(totalCodeSize / 2 ** 20)} MB`);
