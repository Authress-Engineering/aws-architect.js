...
class ApiGatewayManager {
  ...
  async DeployStagePromise(apiGateway, stageName, stage, lambdaVersion) {
    try {
      if (apiGateway.legacy) {
        const success = await this.apiGatewayFactory.createDeployment({
      ...
      try {
        ...
        await this.apiGatewayFactoryV2.createStage({
          ApiId: apiGateway.id,
          StageName: stageName,
          AutoDeploy: true,
          StageVariables: {
      ...
      const success = await this.apiGatewayFactoryV2.createDeployment({
        ApiId: apiGateway.id,
        StageName: stageName,
        Description: `${stage} (lambdaVersion: ${lambdaVersion})`
      }).promise();
